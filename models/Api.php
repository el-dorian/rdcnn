...
// обработаю файл
    ...
    private static function getCenterInfo(): array
    {
        ...
        if (!empty($authToken) && self::token_valid($authToken)) {
            ...
            if (!empty($request->bodyParams['date'])) {
                $info = ExecutionHandler::getTodayCenterInfo($request->bodyParams['date']);
            } else {
    ...
    private static function doArchiveSearch(): array
    {
        ...
        if (!empty($authToken) && self::token_valid($authToken)) {
            ...
            $doctor = urldecode($request->bodyParams['doctor']);
    ...
    private static function registerExecution(): array
    {
        $request = Yii::$app->getRequest();
        $authToken = $request->bodyParams['token'];
        if (empty($authToken) || !self::token_valid($authToken)) {
            return ['status' => false, 'token' => $authToken];
        }
        $executionId = $request->bodyParams['execution_number'];
        Telegram::sendDebug("request register execution $executionId");
        return ['status' => true];
    }
}
...
class ExecutionHandler extends Model
{
    ...
    public static function deleteAddConcs($id, bool $saveToArchive = false): void
    {
            ...
            foreach ($existentFiles as $existentFile) {
                if (preg_match($searchPattern, $existentFile)) {
                    ...
                    if (is_file($path)) {
                        ...
                        if (is_file($nbPath)) {
                            if ($saveToArchive) {
                                Telegram::sendDebug("add additional conc $nbPath");
                                // асинхронно добавлю файл в архив
                                $file = Yii::$app->basePath . '\\yii.bat';
                                if (is_file($file)) {
                                    $command = "$file console/archive $nbPath";
                                    exec($command);
                                }
                            } else {
                                try {
...
// проверю, зарегистрирован ли пользователь с данным именем. Если нет- зарегистрирую
    ...
    public static function getTodayCenterInfo(?string $date = null): array
    {
        $answer = [];
        $dayStart = TimeHandler::getTodayStart();
        // get executions registered for this center for today
        if ($date === null) {
            $registered = User::find()->where(['>', 'created_at', $dayStart])->all();
        } else {
            $interval = TimeHandler::getDayStartAndFinish($date);
            $registered = User::find()->where(['>', 'created_at', $interval['start']])->andWhere(['<', 'created_at', $interval['end']])->all();
        }
        if (!empty($registered)) {
    ...
    public static function getDailyInfo(string $date, int $center): array
    {
        $interval = TimeHandler::getDayStartAndFinish($date);
        $registered = User::find()->select(['id', 'username', 'status', 'updated_at', 'created_at', 'failed_try', 'last_login_try'])->where(['>', 'created_at', $interval['start']])->andWhere(['<', 'created_at', $interval['end']])->all();
        $answer = [];
        if (!empty($registered)) {
            // filter registered
            foreach ($registered as $key => $item) {
                ...
                else if ($center === self::CENTER_CT && !str_starts_with($item->username, "T")) {
                    unset($registered[$key]);
                }
                else if ($center === self::CENTER_NV && !ctype_digit(substr($item->username, 0, 1))){
                    unset($registered[$key]);
                }
            }
            foreach ($registered as $item) {
                $answerItem = [];
                $answerItem['examination'] = $item;
                $mailInfo = Emails::findAll(['patient_id' => $item->id]);
                $answerItem['mails'] = $mailInfo;
                $conclusions = Conclusion::find()->where(['execution_number' => $item->username])->select(['id', 'execution_number', 'execution_date', 'diagnostician', 'patient_name', 'patient_birthdate', 'patient_sex', 'execution_area', 'contrast_info'])->all();
                $answerItem['conclusions'] = $conclusions;
                $execution = DicomArchive::find()->where(['execution_number' => $item->username])->select(['id', 'execution_number'])->one();
                $answerItem['dicom'] = $execution;
                if (!empty($conclusions)) {
                    $patientName = $conclusions[0]->patient_name;
                    $patientBirthdate = $conclusions[0]->patient_birthdate;
                }
                if (!empty($patientName)) {
                    $answerItem['previousExecutionsCount'] = (string)Archive_complex_execution_info::countPreviousExecutions($patientName, $patientBirthdate);
                } else {
                    $answerItem['previousExecutionsCount'] = "0";
                }
                $answer[] = $answerItem;
            }
        }
    ...
    #[ArrayShape(['status' => "int", 'message' => "string"])] public function register(): ?array
    {
        if ($this->validate()) {
            ...
            $user = User::findByUsername($this->executionNumber);
            if ($user !== null) {
                if ($user->status === User::USER_ARCHIVED) {
                    $script = "
<script>
    let reactiveBtn = $('#reactiveBtn');
    reactiveBtn.on('click.reactive', function() {sendAjax('get', '/enable/$user->username', simpleAnswerHandler)});
    let reactiveWithNewPassBtn = $('#reactiveWithNewPassBtn');
    reactiveWithNewPassBtn.on('click.reactive', function() {sendAjax('get', '/enable/$user->username/1', simpleAnswerHandler)});
</script>";
                    return ['status' => 4, 'message' => 'Учётная запись пользователя архивирована.<br/><div class="btn-group-vertical"><button class="btn btn-info" id="reactiveBtn">Активировать со старым паролем</button><button class="btn btn-info" id="reactiveWithNewPassBtn">Активировать и сменить пароль</button></div>' . $script];
                }

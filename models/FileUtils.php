...
class FileUtils
{
    ...
    public static function addBackground($source, $destination, $executionNumber)
    {
        if (str_starts_with($executionNumber, "T")) {
            $pdfBackgroundImage = Yii::$app->basePath . '\\design\\back_ct.jpg';
        } else {
            $pdfBackgroundImage = Yii::$app->basePath . '\\design\\back.jpg';
        }
        if (is_file($source) && is_file($pdfBackgroundImage)) {
            $pdf = new Fpdi();
    ...
    public static function saveScheduleFile(string $downloadedFile): void
    {
        ...
        Telegram::sendDebug("saved file size " . filesize($root . "/schedule/$fileName"));
    }

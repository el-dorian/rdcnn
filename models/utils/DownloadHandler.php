...
class DownloadHandler
{
     ...
     * @throws NotFoundHttpException
     */
    public static function uploadConclusionFile(Conclusion $conclusion): void
    {
        $path = Info::CONC_FOLDER . $conclusion->path_to_file;
        Telegram::sendDebug("request load $path");
        if (is_file($path)) {
     ...
     * @throws NotFoundHttpException
     */
    public static function uploadConclusionFileWithoutBackground(Conclusion $conclusion): void
    {
        $path = Info::CONC_FOLDER . $conclusion->path_to_file;
        Telegram::sendDebug("request load $path");
        if (is_file($path)) {
            Yii::$app->response->sendFile($path, "Заключение МРТ №$conclusion->execution_number $conclusion->execution_area.pdf");
        } else {
            throw new NotFoundHttpException();
        }
    }
     ...
     * @throws NotFoundHttpException
     */
    public static function uploadDicom(DicomArchive $dicomArchive): void
    {
        $path = Info::EXEC_FOLDER . $dicomArchive->path_to_file;
        Telegram::sendDebug("request load $path");
        if (is_file($path)) {
            Yii::$app->response->sendFile($path, "Архив DICOM $dicomArchive->execution_number.zip");;
        } else {
            throw new NotFoundHttpException();
        }
    }
     ...
     * @throws NotFoundHttpException
     */
    public static function uploadArchiveConclusionFileWithoutBackground(Archive_execution $conclusion): void
    {
        $path = Info::PDF_ARCHIVE_PATH . $conclusion->path;
        Telegram::sendDebug("request load $path");
        if (is_file($path)) {
            Yii::$app->response->sendFile($path, "Заключение МРТ №$conclusion->execution_number $conclusion->execution_area.pdf");
        } else {

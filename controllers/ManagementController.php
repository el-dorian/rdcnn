...
class ManagementController extends Controller
{
                ...
                'rules' => [
                    [
                        ...
                        'actions' => [
                            ...
                            'handle-mail',
                            'get-patient-mail-add-form',
                            'get-patient-mail-change-form',
                            'add-mail',
                            'change-mail',
                            'delete-mail',
                            'delete-conclusions',
     ...
     * @return array
     */
    #[ArrayShape(['status' => "int", 'message' => "string"])] public function actionUpdateDependencies(): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    #[ArrayShape(['status' => "int", 'message' => "string"])] public function actionRestartServer(): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
    ...
    public function actionGetPatientMailAddForm($examinationId): ?array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $patient = User::findIdentity($examinationId);
        if ($patient !== null) {
            $existentMail = Emails::findOne(['patient_id' => $patient->id]);
            if ($existentMail === null) {
                ...
                if (!empty($existentMail)) {
                    foreach ($existentMail as $mail) {
                        $model->addresses[] = $mail->address;
                    }
                }
                return ['status' => true, 'header' => 'Добавление адреса электронной почты', 'view' => $this->renderAjax('add-patient-mail-form', ['model' => $model])];
            }
            // предложу заменить адрес или удалить почту
            return ['status' => false, 'message' => 'У пациента уже имеется адрес почты'];
        }
        return null;
    }

    public function actionGetPatientMailChangeForm($examinationId): ?array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $patient = User::findIdentity($examinationId);
        if ($patient !== null) {
            $existentMail = Emails::findAll(['patient_id' => $patient->id]);
            if (!empty($existentMail)) {
                $model = new Emails();
                $model->patient_id = $patient->id;
                $compiledEmailsText = '';
                foreach ($existentMail as $mail) {
                    $compiledEmailsText .= " $mail->address";
                }
                $model->address = $compiledEmailsText;
                return ['status' => true, 'header' => 'Изменение электронной почты', 'view' => $this->renderAjax('change-patient-mail-form', ['model' => $model])];
            }
            // предложу заменить адрес или удалить почту
            return ['status' => false, 'message' => 'Нет зарегистрированных адресов электронной почты'];
        }
        return null;
    }
    ...
    public function actionArchive(): void
    {
        if (Yii::$app->request->isAjax) {
            Yii::$app->response->format = Response::FORMAT_JSON;
            $conclusionFile = 'C:\pdf\1.pdf';
            $file = Yii::$app->basePath . '\\yii.bat';
            if (is_file($file)) {
                $command = "$file console/archive $conclusionFile";
                file_put_contents(Yii::$app->basePath . "/logs/archive.log", time() . " $command\n", FILE_APPEND);
                exec($command);
            }
        }
    }
